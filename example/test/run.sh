...
own_elfloader_kernel_image=$(pwd)/../../../unikraft/elfloader/workdir/build/elfloader_qemu-x86_64
...
qemu-system-x86_64 -kernel ${elfloader_strace_kernel_image_in_run_app_elfloader} -nographic -m 256M -append "${cmd}" -fsdev local,id=myid,path=$(pwd)/file-system,security_model=none -device virtio-9p-pci,fsdev=myid,mount_tag=fs1,disable-modern=on,disable-legacy=off -cpu max -enable-kvm