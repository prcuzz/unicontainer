...
qemu-system-x86_64 -m 2G -nographic -nodefaults -display none -serial stdio -device isa-debug-exit -fsdev local,security_model=none,id=hvirtio0,path=$(pwd)/file-system/ -device virtio-9p-pci,fsdev=hvirtio0,mount_tag=fs1 -kernel ${my_own_elfloader_kernel_image} -cpu max -enable-kvm -net nic,model=virtio,macaddr=52:54:00:12:34:30 -net bridge,br=virbr0 -append "netdev.ipv4_addr=172.44.0.3 netdev.ipv4_gw_addr=172.44.0.1 netdev.ipv4_subnet_mask=255.255.255.0 -- /socket_unix_b \tmp"
